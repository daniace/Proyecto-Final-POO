...
class ABMUsuarios:
    def __init__(self):
        ...
        pygame.display.set_caption("ABM de Carta")
        ...
        self.main_window.add(gui.Label("ID Carta (para eliminar/modificar):"), 10, 10)
        self.main_window.add(self.entry_id, 200, 10)
        self.main_window.add(gui.Label("Nombre Jugador :"), 10, 40)
        self.main_window.add(self.entry_nombre, 200, 40)
        self.main_window.add(gui.Label("Nacionalidad:"), 10, 70)
        self.main_window.add(self.entry_email, 200, 70)
        self.main_window.add(gui.Label("Club:"), 10, 100)
        ...
        self.main_window.add(gui.Label("Cartas:"), 10, 170)
    ...
    def obtener_usuarios(self):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT id_carta, short_name, nationality, club_name FROM carta LIMIT 50"
        )
    ...
    def agregar_usuario(self, btn=None):
        ...
        if nombre and email and telefono:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO carta (short_name, nationality, club_name) VALUES (?, ?, ?)",
    ...
    def eliminar_usuario(self, btn=None):
        ...
        if id_usuario:
            conn = sqlite3.connect("usuarios.db")
            cursor = conn.cursor()
            cursor.execute("DELETE FROM carta WHERE id_carta = ?", (id_usuario,))
            conn.commit()
            conn.close()
            self.actualizar_lista_usuarios()
    ...
    def modificar_usuario(self, btn=None):
        ...
        if nombre:
            cursor.execute(
                "UPDATE carta SET short_name = ? WHERE id_carta = ?",
                (nombre, id_usuario),
            )
        ...
        if email:
            cursor.execute(
                "UPDATE carta SET nationality = ? WHERE id_carta = ?",
                (email, id_usuario),
            )
        ...
        if telefono:
            cursor.execute(
                "UPDATE carta SET club_name = ? WHERE id_carta = ?",
                (telefono, id_usuario),
            )
